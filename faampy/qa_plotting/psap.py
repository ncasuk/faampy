...
def plot_psap_flo(ax,data):
    ...
    ax.plot_date(data['mpl_timestamp'][:,0].ravel(),
                 data['PSAP_FLO'][:].ravel(), '-', color='#ff4d4d', label = 'psap_flo')
...
def main(ds):
    ...
    ax0 = plot_psap(fig.get_axes()[0], data)
    ax1 = plot_psap_flo(fig.get_axes()[1], data)
    ax2 = plot_psap_lin_flag(fig.get_axes()[2], data)
