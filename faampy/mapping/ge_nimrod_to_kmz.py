...
The original UKMO nimrod data files can be found at the badc in:

  http://badc.nerc.ac.uk/browse/badc/ukmo-nimrod/data/composite/uk-1km/

The script is very wasteful in terms of disk space. The temporary folder
that is created for storing the image files can grow to several GB. The script
deletes the temporary folder by default.

At the moment the script only works with the 1km composite data, but it should be
straight forward to adapt the script for other rain radar data files that are supplied
in nimrod format.

Due to the fact that this script makes use of many common linux command line
tools (zip, tar, gunzip, gdal_translate, gdalwarp) it is unlikely that it will
