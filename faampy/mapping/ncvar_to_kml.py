...
def get_run_kml(run_data, ds, var, offset, scale_factor, time_lag):
    ...
    if len(ds.variables[var][:].shape) > 1:
        alt=(ds.variables[var][:][s_index+time_lag:e_index+time_lag, 0]+offset)*scale_factor
    else:
        alt=(ds.variables[var][:][s_index+time_lag:e_index+time_lag]+offset)*scale_factor
    ...
    #alt_flag=ds.variables[var+'_FLAG'][:][s_index+time_lag:e_index+time_lag, 0]
    alt[alt_flag != 0]=0
    alt=list(alt)
...
def process(ncfile, ncvar, time_lag, offset, scale_factor, out_path, _RUNS):
    ...
    kml_filename=os.path.join(out_path, fid + '-' + datestring + ncvar.lower() + '.kml')
    kml=open(kml_filename, 'w')
    #kml.write(KML_HEADER % (fid + '-' + os.path.basename(ncfile).split('_')[2]))
...
def main():
    ...
    process(args.faam_core_netcdf, args.ncvar, args.time_lag, args.offset, args.scale_factor, args.outpath, _RUNS)
