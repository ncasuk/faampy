...
GPX_ files are a standard xml-format, which is understood by many programs.
The gpx file from a flight can for example be used to georeference photographs
taken during a flight (e.g. gpscorrelate_).

The geotagged images can then in a next step be overlaid on maps using the
GPS information from the image metadata.

.. _GPX: https://en.wikipedia.org/wiki/GPS_Exchange_Format
.. _gpscorrelate: https://github.com/freefoote/gpscorrelate
