...
def get_mpl_time(ds, *freq):
    """Return a matplotlib usable time format from the core faam Dataset
...
def core_to_pandas(ds):
    """converts a netCDF4.Dataset into a pandas Dataframe using the timestamp as index.
    Only the first column of the two dimensional data set is grabbed, because of performance
    issues
